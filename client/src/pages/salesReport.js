...
export default function SalesReport() {
  ...
  const [startDate, setStartDate] = useState("invalid");
  const [endDate, setEndDate] = useState("invalid");
  const [orderSales, setOrderSales] = useState([]);
  const [reportGenerated, setReportGenerated] = useState(false);
  const [drinksData, setDrinksData] = useState();
  const [chipsData, setChipsData] = useState();
  const [chipsSearchQuery, setChipsSearchQuery] = useState("");
  ...
  const handleSubmit = (event) => {
    const url = port + "/sales/" + startDate + "/" + endDate;
    console.log(url);
        ...
        <form  style={{marginLeft: "30%", marginTop: "1%"}}onSubmit={handleSubmit}>
          <label style={{paddingRight: "5%  "}}>
            ...
            style={{marginLeft: "10%", marginTop: "1%"}}
              ...
              onChange={(e) => {
                if (e.target.value === "") {
                  setStartDate("blank"); // Set a default value when the input field is cleared
                } else {
          ...
          <label>
          <br></br><br></br>Enter the end date 
            <input
              type="date"
              value={endDate}
              style={{marginLeft: "10%", marginTop: "1%"}}
              ...
              onChange={(e) => {
                if (e.target.value === "") {
                  setEndDate("blank"); // Set a default value when the input field is cleared
                } else {
      ...
      {reportGenerated && (
        <div class="flex justify-end table-container">
          <div class="w-full overflow-x-scroll">
            <h2 style={{ marginTop: "3%", marginLeft: "50%" }}>Items Sold</h2>
            <input
              type="text"
              value={searchQuery}
